...
@RestController @RequestMapping("/customers") public class CustomerController
{
  ...
  @PostMapping("/location") public ResponseEntity<List<BusinessInRadiusResponseDto>> updateCustomerLocation(
      @RequestBody CustomerLocationRequestResponseDto dto,
  ...
  @GetMapping("/location") public ResponseEntity<CustomerLocationRequestResponseDto> getCustomerLocation(
      HttpServletRequest request)
  {
    String userId = (String) request.getAttribute("userId");
    CustomerLocationRequestResponseDto location = customerService.getCustomerLocation(
        userId);
    return ResponseEntity.ok(location);
  }

  @GetMapping("/radius-businesses") @PreAuthorize("hasAnyAuthority('BUSINESS', 'CUSTOMER')") public ResponseEntity<List<BusinessInRadiusResponseDto>> getBusinessesInRadius(
      HttpServletRequest request)
  {
    String userId = (String) request.getAttribute("userId");
    List<BusinessInRadiusResponseDto> businessesInRadius = customerService.getBusinessesInRadius(
        userId);
    return ResponseEntity.ok(businessesInRadius);
  }

  @PutMapping public ResponseEntity<?> updateCustomerProfile(
      ...
      BindingResult bindingResult)
  {
    ...
    if (bindingResult.hasErrors())
    {
      ...
      Map<String, String> errors = bindingResult.getFieldErrors().stream()
          .collect(Collectors.toMap(FieldError::getField,
              FieldError::getDefaultMessage, (existing, replacement) -> existing
    ...
    {
      // Business logic
      CustomerResponseDto responseDto = customerService.updateCustomerProfile(
          updatedProfile);
